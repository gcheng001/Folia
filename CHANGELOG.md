...
- 新增可编辑 SVG 场景 v1：Folia 生成的 SVG 在普通看图软件中仍是标准成品，在 Folia 中可恢复节点、连接、样式、来源与历史；支持拖动、文字/坐标/尺寸/字号/颜色修改、节点和连接增删、连接线跟随、撤销重做、重开恢复历史和局部自动消重叠。
- 普通旧 SVG 可升级为“可编辑副本”，原件永不覆盖。升级首先保持原有可见 SVG 不变，按置信度映射旧方框、文字和连接；后续编辑只定点修改已识别元素，未识别装饰继续原样保留。
- 新增 Markdown—图表对照工作台：左侧继续编辑 Markdown，右侧查看、编辑或重新生成 SVG；生成完成后可选自动进入对照，左右文档独立自动保存，`Cmd+S` 同时保存两侧，关闭和重开软件会恢复上次对照状态。
- 新增图表质量与事实门：检查文字溢出、节点/旧文字重叠、越界、穿线和过小字体，并对当前 Markdown 中的日期、金额、案号与原文锚点做精确核对；未通过时阻止交付 SVG 导出。
- 新增交付 SVG 净化：导出副本会移除可编辑场景、历史、内部元素编号、脚本、事件属性和外链。
- 新增独立“Skill 成品图”入口：调用随 Folia 锁版的 `baoyu-diagram` 规则生成静态 SVG，支持流程图、时间轴、关系图、脑图，以及浅色正式、简洁商务、深色科技、柔和彩色四种可预览风格；软件提供可覆盖的类型推荐，并按文档记住风格。
- Skill 成品图始终使用编辑器最新内容，准确提示内容会经 Claude Code 提交给 Claude 服务；生成过程展示真实阶段和等待时间并支持取消。结果先写临时文件、通过自包含 SVG 检查后再按“原文名称-图类型-版本号”保存，旧版本不覆盖。
- Folia 可直接打开和拖放 `.svg` 成品图，提供缩放、事实核对提醒和“另存副本”；内置 Skill 的 MIT 许可文本随安装包分发。
- 在保留原脑图入口的同时，新增独立的本地离线“一键可视化”：根据 Markdown 的明确结构自动推荐并生成独立、可保存的 `.foliaviz` 工作簿，覆盖结构总览、时间轴、关系网络、流程与决策、矩阵与对比、数值图表六类视图。
- 可视化正式元素携带来源锚点；展示文字、布局与视图注释独立保存。只有用户查看差异并确认“同步修正原文”后才回写绑定 Markdown，外部原文变化通过增量审阅更新并尽量保留布局与展示覆盖。
- 可视化工作簿支持 PNG、适用页签 SVG、PDF 和独立 HTML 导出；交付文件使用 Canvas-safe 色板并移除源路径、hash、锚点摘录和内部元素 ID。
- 新增 `.foliaviz` 文件打开、拖放、保存、系统文件关联与 UTF-8/10MB 安全边界支持。
...
- Skill 成品图生成弹层支持收起后后台继续，期间可返回 Markdown 阅读编辑，并可点击工具栏运行图标重新展开；Esc 不再误取消任务。生成调用固定使用 Claude Code 的 `haiku` 快速档、低推理强度和专用短 system prompt，并改为流式读取 SVG、展示实际模型、保留受限错误输出；本机同一份 11.7KB 真实文档由 5 分钟未完成缩短为约 50-60 秒完成。
...
- 修复两份真实法律 SVG 中的节点重叠和同框文字同坐标重叠：局部修复现在既移动冲突节点，也重排旧图内部文字基线并按需扩展方框；两份指定样本均有“先复现、修复后零重叠”的自动回归。
- 修复 Vditor 锁定复杂 HTML 表格时，表格后新增正文虽然显示在编辑器中却没有同步回 Markdown 状态的问题；增加原生 `input` 兜底读取最终 Vditor 值。
- 恢复外观设置中的状态栏路径显示方式（仅文件名 / 中间省略 / 完整路径）及非法配置回退，并修正内测授权 E2E 使用已生效的授权码。
- 修复即时渲染编辑器中点击或继续编辑标题时 Markdown `#` 反复闪现的问题：此前 220ms 定时器只移除一次 Vditor 的展开 class，选区或输入变化会再次展开。现在保留标题 marker DOM 供 Markdown 序列化，但在编辑器视觉层永久折叠为 0×0；复杂表格、SVG、Mermaid 和其他需要编辑的 Markdown 标记链路不变，并新增浏览器回归覆盖“刚展开、尚未等到定时器”的真实瞬间。
- 修复 Skill 成品图在 ExFAT 移动硬盘等不支持硬链接的文件系统上生成后必然保存失败的问题：最终版本改用 `create_new` 不覆盖地占用文件名并复制已校验临时 SVG，遇到同名继续递增版本，写入失败清理残缺候选，旧成品保持不变。
- 恢复被一键可视化误占用的脑图入口；脑图与一键可视化改为两个独立按钮。时间轴现在优先读取“时间线/客观事实”等语义章节，只保留事实事件并按标准化日期升序排列、去除分析性重复；关系网络支持正文中的明确箭头关系，流程支持“第一层/第二层/第三层”等检视程式。新增或重新打开的工作簿会后台读取记录的原文路径并升级抽取规则，不再停留在“等待重新绑定原文”。
- 修复脑图 PNG/PDF 导出遇到 `oklch()` 时直接失败的问题：导出副本改用 Canvas-safe 色板，清洗 html2canvas 不支持的颜色属性，并用专属标记定位截图副本；脑图网格的直接颜色值同步改为兼容格式。
- 脑图画布新增自由画线工具：可在“无连接线”模式下手动画箭头或线段，端点支持自由落点、靠近节点吸附/绑定、Shift 角度约束；自由线可选中、删除、改颜色/箭头/虚线/粗细，并纳入导出范围。新增一键规整选中对象（无选择时规整画布）和 XS/S/M/L/XL 节点尺寸档位，节点颜色与尺寸取消选择后仍会持久保留。
- 脑图画布新增“清除手动画线”，可一键删除用户自由绘制的箭头/线段，保留节点、节点连接线、标注框、节点颜色/尺寸和自动连接线模式；同时加强 PNG/PDF 导出克隆，避免自由线 SVG 外溢和编辑态 UI 影响导出。
- 修复脑图画布的 sidecar 兼容与交互稳定性：节点位置、样式、标注框和自定义连线改用更稳定的节点引用并兼容旧位置缓存；撤销/重做保存 Markdown + sidecar 快照；边和标注框可通过点击选中后删除；PNG/PDF 导出会按真实画布包围盒裁剪并剔除编辑态 UI。
- 修复脑图画布流程图编辑链路：连接模式下显示可拖拽连接点，“无连接线”只隐藏自动父子线、不影响手动自定义箭头；节点颜色改为持久填充色，取消选择后仍保留；节点、标注框和自定义边增加明确选中反馈；导出克隆会清理选择光晕、连接点和不稳定 CSS，降低 PNG/PDF 导出失败概率。
- 脑图节点文字现在可在 macOS WKWebView 中稳定双击进入行内编辑；输入框隔离 React Flow 的拖拽、缩放和平移手势，修改后按 Enter 或点击空白处会同步回 Markdown。文件名生成的虚拟中心节点也可编辑，首次修改会在 Markdown 的 frontmatter 后自动落成真实 H1。
- 重做脑图直接操作：单击选中后按 Space 编辑，连续点击同一节点第二下立即编辑；键盘命令改为脑图页面级监听，根节点 Enter 创建子节点，普通节点 Enter 创建同级、Tab 创建下级。节点视觉统一为小圆角长方形完整边框，连接线统一为单段直线。
- 新增默认「经典树」主题：黑色实心根节点、浅灰圆角子节点，以及水平主干、共享竖干和圆角直角支路；原有蓝色/彩色/紫色/黑白直线主题继续保留。可编辑脑图节点支持鼠标自由拖动，位置按文件路径与节点内容路径保存到本地布局 sidecar，不改写 Markdown 层级。
