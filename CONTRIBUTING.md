...
`.github/workflows/ci.yml` 会在 push / PR 时自动跑 `npm ci` → `npm run typecheck` → `npm run lint` → `npm test` → `npm run build`（ISS-173）。**CI 不通过的 PR 不合入**——这条硬性 gate 是 v0.4.1 hotfix（DEC-106）复盘后新增的，前端测试套件曾因无 CI 拦截而把 ISS-171 的 43 个失败一路放行到 release。`cargo test` 与 `tauri build` 由 release workflow 负责，本仓 CI 只覆盖 npm 链路。
